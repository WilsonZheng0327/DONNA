...
│ VL53L0X ToF├───────┤ node MCU│ ))) 13 bytes ((( │ hub: XIAO    ├─────────┤ Firebase ├──────┤ dashboard │
...
Name desks on the board (optional):
`curl -X PATCH "https://HOST/config/desks/node-1.json?auth=SECRET" -d '{"name":"Window desk"}'`
